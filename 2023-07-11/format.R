...
font_add(family = "fa-brands", regular = "/Users/tyleroldham/Documents/Data Viz Projects/TidyTuesday/fonts/Font Awesome 6 Brands-Regular-400.otf")
...
ggplot(data = df, aes(x = date, y = value)) +
  ...
  theme_minimal(base_family = heading_font) +
  ...
  theme(
    ...
    axis.text = element_text(family = p_font, margin = margin(r = 5)),
    
    plot.title    = element_text(size = 32, family = heading_font),
    plot.subtitle = element_markdown(margin = margin(b = 15), size = 11, color = "grey20", family = p_font),
    plot.caption  = element_markdown(margin = margin(t = 15), size = 6, family = p_font),
  ...
  labs(
    ...
    caption = paste0(
      folder_icon, "NASA GISS Surface Temperature Analysis (GISTEMP v4) | ",
      twitter_icon, "Nikolaos Pechlivanis"
    )
  )
